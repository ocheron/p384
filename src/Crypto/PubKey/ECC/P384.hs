...
-- | Get the base point for the P384 Curve
pointBase :: Point
pointBase =
...
toPoint s
    ...
    | otherwise      =
        withNewPoint $ \px py -> withScalar s $ \p ->
            ccryptonite_p384_basepoint_mul p px py

-- | Add a point to another point
pointAdd :: Point -> Point -> Point
pointAdd a b = withNewPoint $ \dx dy ->
    withPoint a $ \ax ay -> withPoint b $ \bx by ->
        ccryptonite_p384e_point_add ax ay bx by dx dy
...
pointMul scalar p = withNewPoint $ \dx dy ->
    withScalar scalar $ \n -> withPoint p $ \px py -> withScalarZero $ \nzero ->
        ccryptonite_p384_points_mul_vartime nzero n px py dx dy
...
pointDh scalar p =
    B.unsafeCreate scalarSize $ \dst -> withTempPoint $ \dx dy -> do
        withScalar scalar $ \n -> withPoint p $ \px py -> withScalarZero $ \nzero ->
            ccryptonite_p384_points_mul_vartime nzero n px py dx dy
        ccryptonite_p384_to_bin (castPtr dx) dst
...
pointsMulVarTime n1 n2 p = withNewPoint $ \dx dy ->
    withScalar n1 $ \pn1 -> withScalar n2 $ \pn2 -> withPoint p $ \px py ->
        ccryptonite_p384_points_mul_vartime pn1 pn2 px py dx dy
...
pointToIntegers p = unsafeDoIO $ withPoint p $ \px py ->
    allocTemp 48 (serialize (castPtr px) (castPtr py))
  where
    serialize px py temp = do
        ccryptonite_p384_to_bin px temp
        x <- os2ip temp scalarSize
        ccryptonite_p384_to_bin py temp
...
pointFromIntegers (x,y) = withNewPoint $ \dx dy ->
  ...
  where
    -- put @n to @temp in big endian format, then from @temp to @dest in p384 scalar format
    fill :: Ptr Word8 -> Ptr P384Scalar -> Integer -> IO ()
    fill temp dest n = do
        ...
        -- then fill dest with the P384 scalar from temp
        ccryptonite_p384_from_bin temp dest
...
pointToBinary p = B.unsafeCreate pointSize $ \dst -> withPoint p $ \px py -> do
    ccryptonite_p384_to_bin (castPtr px) dst
    ccryptonite_p384_to_bin (castPtr py) (dst `plusPtr` 48)
...
unsafePointFromBinary ba
    ...
    | otherwise                =
        CryptoPassed $ withNewPoint $ \px py -> B.withByteArray ba $ \src -> do
            ...
            ccryptonite_p384_from_bin (src `plusPtr` scalarSize) (castPtr py)
...
scalarGenerate = unwrap . scalarFromBinary . witness <$> getRandomBytes 48
  where
...
scalarIsZero s = unsafeDoIO $ withScalar s $ \d -> do
    result <- ccryptonite_p384_is_zero d
    return $ result /= 0
...
scalarAdd a b =
    withNewScalarFreeze $ \d -> withScalar a $ \pa -> withScalar b $ \pb ->
        ccryptonite_p384e_modadd ccryptonite_SECP384r1_n pa pb d
...
scalarToBinary s = B.unsafeCreate scalarSize $ \b -> withScalar s $ \p ->
    ccryptonite_p384_to_bin p b
{-# NOINLINE scalarToBinary #-}

-- | Convert from an Integer to a P384 Scalar
scalarFromInteger :: Integer -> CryptoFailable Scalar
scalarFromInteger i =
    maybe (CryptoFailed CryptoError_SecretKeySizeInvalid) scalarFromBinary (S.i2ospOf 48 i :: Maybe Bytes)

-- | Convert from a P384 Scalar to an Integer
...
-- Foreign bindings
------------------------------------------------------------------------
foreign import ccall "&cryptonite_SECP384r1_n"
    ccryptonite_SECP384r1_n :: Ptr P384Scalar

foreign import ccall "cryptonite_p384_init"
    ccryptonite_p384_init :: Ptr P384Scalar -> IO ()
foreign import ccall "cryptonite_p384_is_zero"
    ccryptonite_p384_is_zero :: Ptr P384Scalar -> IO CInt
foreign import ccall "cryptonite_p384e_modadd"
    ccryptonite_p384e_modadd :: Ptr P384Scalar -> Ptr P384Scalar -> Ptr P384Scalar -> Ptr P384Scalar -> IO ()
foreign import ccall "cryptonite_p384e_modsub"
...
foreign import ccall "cryptonite_p384_modinv_vartime"
    ccryptonite_p384_modinv_vartime :: Ptr P384Scalar -> Ptr P384Scalar -> Ptr P384Scalar -> IO ()
foreign import ccall "cryptonite_p384_base_point_mul"
    ccryptonite_p384_basepoint_mul :: Ptr P384Scalar
                                   -> Ptr P384X -> Ptr P384Y
                                   -> IO ()

foreign import ccall "cryptonite_p384e_point_add"
    ccryptonite_p384e_point_add :: Ptr P384X -> Ptr P384Y
                                -> Ptr P384X -> Ptr P384Y
                                -> Ptr P384X -> Ptr P384Y
                                -> IO ()

foreign import ccall "cryptonite_p384e_point_negate"
    ccryptonite_p384e_point_negate :: Ptr P384X -> Ptr P384Y
                                   -> Ptr P384X -> Ptr P384Y
                                   -> IO ()

-- compute (out_x,out,y) = n1 * G + n2 * (in_x,in_y)
foreign import ccall "cryptonite_p384_points_mul_vartime"
    ccryptonite_p384_points_mul_vartime :: Ptr P384Scalar -- n1
                                        -> Ptr P384Scalar -- n2
                                        -> Ptr P384X -> Ptr P384Y -- in_{x,y}
                                        -> Ptr P384X -> Ptr P384Y -- out_{x,y}
                                        -> IO ()
foreign import ccall "cryptonite_p384_is_valid_point"
    ccryptonite_p384_is_valid_point :: Ptr P384X -> Ptr P384Y -> IO CInt

foreign import ccall "cryptonite_p384_to_bin"
    ccryptonite_p384_to_bin :: Ptr P384Scalar -> Ptr Word8 -> IO ()

foreign import ccall "cryptonite_p384_from_bin"
    ccryptonite_p384_from_bin :: Ptr Word8 -> Ptr P384Scalar -> IO ()
